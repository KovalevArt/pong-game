...
class Rect {
	...
	get left ()
	{
		return this.pos.x - this.size.x / 2 - 2
	}
	get right ()
	{
		return this.pos.x + this.size.x / 2 + 2
	...
	get top ()
	{
		return this.pos.y - this.size.y / 2 - 2
	}
	get bottom ()
	{
		return this.pos.y + this.size.y / 2 + 2
...
class Pong 
{
	constructor(canvas)
	{
		...
		this.CHARS = [
        	...
        	str.split('').forEach((fill, i) => {
        		if (fill === '1') {
        			ctx.fillRect((i % 3) * this.CHAR_PIXEL, (i / 3 | 0) * this.CHAR_PIXEL
	...
	drawScore()
	{
		...
		const CHAR_W = this.CHAR_PIXEL * 4; // 3 для цифры и 4 пиксель для пробела
		this.players.forEach((player, i) => {
			const chars = player.score.toString().split('');
			const offset = align *
						 (i +1) - 
						 (CHAR_W * chars.length / 2) + // длина трети канвас поля * на номер игрока*( первый или второй) - пиксели для прорисовки самой цифры  + смещение вправо на @язапуталсятут@ знает что
						this.CHAR_PIXEL / 2;
